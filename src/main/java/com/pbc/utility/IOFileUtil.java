...
public class IOFileUtil {
	 ...
	 * @return
	 */
	// public <T extends Serializable> String writObjectLocally(final byte[]
	// dataBytes, final String fileName) {
	// FileOutputStream fos;
	// final String completeFilePath = getCompletePath(fileName,
	// getCorrectFolderPath());
	// try {
	// fos = new FileOutputStream(completeFilePath);
	// fos.write(dataBytes);
	// fos.close();
	// } catch (final IOException e) {
	// logger.error("Object could not be saved on local files because of some
	// I/O error", e);
	// throw new BlockProcessingException("Object could not be saved on local
	// files", e);
	// }
	// return completeFilePath;
	// }

	public String writObjectLocally(final InputStream stream, final String combineKey) {
		final String completeFilePath = getCompletePath(combineKey, getCorrectFolderPath());
		try {
