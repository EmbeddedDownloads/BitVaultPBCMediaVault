...
public class SingleFileJsonPersistor<T> implements Persistor<T> {
	...
	/**
	 * List to track which block was actually inserted after which one. In absence
	 * of this list it will be hard(Obviously there are ways but not faster than
	 * this one) to find out which is the actual list containing actual data.
	 */
	...
	public T getBlock(final String hash) {
		...
		try {
			...
			if (locationOfBlock == null) {
				...
				logger.info("Location of block is null for " + hash);
				return null;
			}
	...
	public void removeBlockWithHash(final String hash) {
		try {
			 ...
			 *
			 ************ Second block(else if) one is being used in case of last block in block chain.
			 *
			 ************ Third block (else) is used for breaking links as this block is in middle.
			 */
	...
	/**
	 * Runs when node/server start. Initialize all the map and set with block chain
	 * data.
	 */
	@Override
