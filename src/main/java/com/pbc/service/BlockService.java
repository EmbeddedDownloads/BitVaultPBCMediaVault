...
public class BlockService {
	...
	public FileSystemResource getDownloadFile(final String fileId) throws DataException {
		...
		if (filePathAndStatus != null && Boolean.FALSE.equals(filePathAndStatus.getStatus())) {
			...
			try {
				final FileInputStream fileInputStream = new FileInputStream(bufferedFile);
				if (filePathAndStatus.getDataHash().equals(calculateHash(fileInputStream, "SHA-256"))) {
				...
				blockStatusList.forEach(blockStatus -> {
					...
					if (null != block && block.getBlockContent().getTag().equals(TAG_SECURE_MESSAGE)) {
						...
						final BlockStatus status = blockStatusDao.getStatus(TAG_A2A_FILE,
								block.getBlockContent().getHashTxnId());
						logger.info("Get block status for key " + combineKey.toString());
						if (status != null) {
							logger.info("Parsing block to BlockResponseDTO");
							final BlockResponseDTO blockResponseDTO = getBlockResponseDTO(block);
							final TemporaryUrlDownload urlDownload = new TemporaryUrlDownload(
									blockResponseDTO.getFileId(), block.getBlockContent().getFilePath(), false,
									block.getBlockContent().getDataHash());
							tDownloads.add(urlDownload);
							listOfBlocks.add(blockResponseDTO);
						} else {
							logger.info("Block status was null");
						}
					}
	...
	public CrcConfirmMapModel getBothMap() {
		final CrcConfirmMapModel mapModel = new CrcConfirmMapModel();
		mapModel.setConfirmationMap(transactionFactory.getObject().getConfirmationMap());
		mapModel.setCrcMap(TransactionMessageService.getCrcMap());
		return mapModel;
	}
	...
	public boolean isSignatureValid(final String publicKey, final String signature, final String data) {
		return security.isSignatureValid(publicKey, signature, data);
	}
}